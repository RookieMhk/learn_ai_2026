...
2. **分数计算**：$Attention(Q, K, V) = softmax≤ft(QKᵀ/√dₖ)V$
...
线性注意力通过核函数近似将计算复杂度从$O(n²)$降低到$O(n)$。
...
**线性注意力原理**：
1. **核函数近似**：使用特征映射$φ$将点积注意力转换为线性计算
2. **结合律利用**：$φ(Q)(φ(K)ᵀ V)$可先计算$φ(K)ᵀ V$，避免$O(n²)$矩阵
3. **复杂度降低**：从$O(n²d)$降至$O(ndm)$，其中$m$是特征维度
