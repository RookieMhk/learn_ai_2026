...
自注意力（Self-Attention）是注意力机制的一种特殊形式，其中查询、键和值都来自同一输入序列。给定输入序列 $X ∈ ℝⁿ ᵗⁱᵐᵉˢ ᵈ$，其中 $n$ 为序列长度，$d$ 为特征维度，自注意力的计算过程如下：

### 2.1 线性变换
首先，通过三个可学习的权重矩阵 $WQ ∈ ℝᵈ ᵗⁱᵐᵉˢ ᵈₖ$、$WK ∈ ℝᵈ ᵗⁱᵐᵉˢ ᵈₖ$、$WV ∈ ℝᵈ ᵗⁱᵐᵉˢ ᵈᵥ$ 将输入 $X$ 投影为查询 $Q$、键 $K$ 和值 $V$：

$$Q = X WQ, K = X WK, V = X WV$$

其中 $dₖ$ 和 $dᵥ$ 分别为键和值的维度，通常 $dₖ = dᵥ = d$。

### 2.2 注意力分数计算
计算查询 $Q$ 与键 $K$ 的点积，得到注意力分数矩阵 $S ∈ ℝⁿ ᵗⁱᵐᵉˢ ⁿ$：

$$S = Q K^→p$$

### 2.3 缩放与归一化
为了防止点积结果过大导致梯度消失，将注意力分数除以 $√dₖ$，然后通过 softmax 函数进行归一化，得到注意力权重矩阵 $A$：
...
$$Attention(Q, K, V) = softmax≤ft(Q K^→p/√dₖ) V$$

**缩放因子 $√dₖ$ 的作用**：
- 当 $dₖ$ 较大时，点积结果可能变得非常大，导致 softmax 函数的梯度趋近于零（梯度消失）。
- 除以 $√dₖ$ 可以稳定梯度，提高训练效率。
...
$$Qᵢ = Q WᵢQ, Kᵢ = K WᵢK, Vᵢ = V WᵢV$$

其中 $i = 1, , h$，$WᵢQ ∈ ℝᵈ ᵗⁱᵐᵉˢ ᵈₖ$，$WᵢK ∈ ℝᵈ ᵗⁱᵐᵉˢ ᵈₖ$，$WᵢV ∈ ℝᵈ ᵗⁱᵐᵉˢ ᵈᵥ$，且 $dₖ = dᵥ = d/h$。
...
### 6.1 核心思想
对于位置 $m$ 的查询向量 $qₘ$ 和位置 $n$ 的键向量 $kₙ$，RoPE 通过旋转操作引入相对位置信息：

$$qₘ' = R_ₜₕₑₜₐ,ₘ qₘ, kₙ' = R_ₜₕₑₜₐ,ₙ kₙ$$

其中 $R_ₜₕₑₜₐ,ₘ$ 是一个旋转矩阵，$θ$ 是与位置相关的旋转角度。

### 6.2 数学形式
对于二维情况，旋转矩阵定义为：

$$R_ₜₕₑₜₐ,ₘ = {bmatrix} mθ & -mθ mθ & mθ {bmatrix}$$

则注意力分数计算为：

$$(qₘ')^→p kₙ' = qₘ^→p R_ₜₕₑₜₐ,ₙ₋ₘ kₙ$$
